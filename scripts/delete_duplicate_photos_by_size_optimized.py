...
OPTIMIZED VERSION - Uses batch processing and file output for reliability.
"""
...
def get_all_media_items_batch():
    """Get all media items from Photos using efficient batch processing."""
    ...
    try:
        ...
        print("⚠️  Could not get total count")
        return []
    
    # Use temp file for output (more reliable than returning large lists)
    ...
    print(f"   Processing in {total_batches} batches of {batch_size} items...")
    ...
    for batch_num in range(total_batches):
        ...
        script = f'''tell application "Photos"
            set outputFile to POSIX file "{temp_path}"
            ...
            repeat with i from {start_idx} to {end_idx}
                try
                    ...
                    set outputLine to fileName & "|||" & (fileSize as string) & "|||" & itemId & "\\n"
                    write outputLine to fileRef
                end try
            end repeat
        ...
        try:
            ...
            if result.returncode != 0:
                print(f"   ⚠️  Error in batch {batch_num + 1}: {result.stderr[:100]}")
                continue
            ...
            if (batch_num + 1) % 10 == 0 or batch_num == total_batches - 1:
                print(f"   Processed batch {batch_num + 1}/{total_batches} ({len(items):,} items parsed)...")
                
        except subprocess.TimeoutExpired:
            print(f"   ⚠️  Timeout in batch {batch_num + 1}")
            continue
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Delete duplicate photos/videos, keeping only the largest version (OPTIMIZED)'
    )
    parser.add_argument(
        '--dry-run',
        action='store_true',
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help='Minimum size difference (bytes) to consider as duplicate. Default: 0'
    )
    
    args = parser.parse_args()
    
    print("=" * 80)
    print("🗑️  DELETE DUPLICATE PHOTOS/VIDEOS (KEEP LARGEST) - OPTIMIZED")
    print("=" * 80)
    print()
    ...
    # Get all media items
    media_items = get_all_media_items_batch()
